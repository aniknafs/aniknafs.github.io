...
ReduxStore.subscribe(function () {// console.log('store', ReduxStore.getState());
});
/*
    MST Setup
*/
...
var MSTStore = _mobxStateTree.types.model({
  list: _mobxStateTree.types.optional(_mobxStateTree.types.array(_mobxStateTree.types.string), []) // this list is snapshottable

}).volatile(function (self) {
  return {
    // volatile state is observable (same MobX observable)
    volatileList: []
  };
...
var snapshotListener = (0, _mobxStateTree.onSnapshot)(mstStore, function (snapshot) {// console.log('new snapshot: ', snapshot);
});
/* 
    MobX Setup
*/
...
var input = Array.from(Array(10000).keys()).map(function (x) {
  return "Item ".concat(x);
});
console.group('Initial array creation'); //plain mutable

console.time("mutable");
var mutableList = JSON.parse(JSON.stringify(input)); // has restrictions over the data, e.g. no undefined

console.timeEnd("mutable"); //plain immutable

console.time("immutable with spread");

var plainImmutableList = _toConsumableArray(input);

console.timeEnd("immutable with spread"); //ImmutableJS

console.time("immutablejs");
var immutableList = (0, _immutable.List)(input);
console.timeEnd("immutablejs"); //MST
...
console.timeEnd("mst (volatile)"); //MobX
...
console.time("immutable with spread");
plainImmutableList = _toConsumableArray(plainImmutableList).concat([newItem]);
console.timeEnd("immutable with spread"); //Immutablejs 
...
console.group('Removing an item'); //mutable
...
var plainImmutableList2 = _toConsumableArray(plainImmutableList.slice(0, removeIndex)).concat(_toConsumableArray(plainImmutableList.slice(removeIndex + 1)));
...
var tree = Array(10000).keys().map(function (x) {
  return "Item ".concat(x);
});
